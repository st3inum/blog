...
The main idea of this problem is there are maximum $2\sqrt{n}$ different values of $\lfloor \frac{n}{i} \rfloor$.
...
- Case 2: $x>\sqrt{n}$
	...
	- Hence the values of $\lfloor \frac{n}{i} \rfloor$ (where $\sqrt{n} < i \le n$) will be in range $[1,\frac{n}{\sqrt{n}}] = [1,\sqrt{n}]$.

	- $\therefore$ number of values of $\lfloor \frac{n}{i} \rfloor$ will be at most $\sqrt{n}$.
